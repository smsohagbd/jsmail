...
func (s *Server) Start() {
	...
	mux.HandleFunc("/admin/ippool/master/add", webauth.RequireAdmin(ah.AddIPPoolMasterDomainRule))
	mux.HandleFunc("/admin/ippool/master/update", webauth.RequireAdmin(ah.UpdateIPPoolMasterDomainRule))
	...
	mux.HandleFunc("/user/skip-domain", webauth.RequireUser(uh.SkipDomainPage))
	mux.HandleFunc("/user/skip-domain/add", webauth.RequireUser(uh.AddUserSkipDomain))
	mux.HandleFunc("/user/skip-domain/delete", webauth.RequireUser(uh.DeleteUserSkipDomain))
